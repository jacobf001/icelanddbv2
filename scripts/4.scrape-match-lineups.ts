...
function cleanText(s: any) { return String(s ?? "").replace(/\s+/g, " ").trim(); }
...
function findLineupGrid($: cheerio.CheerioAPI) {
  ...
  for (const g of grids) {
    ...
    const hasByrjunar = $g.find("span").filter((_, s) => clean($(s).text()) === "Byrjunarlið").length > 0;
    if (hasHome && hasAway && hasByrjunar) return $g;
  }
  return null;
}

function findLineupListsForLabel($: cheerio.CheerioAPI, $grid: cheerio.Cheerio<any>, label: "Byrjunarlið" | "Varamenn") {
  ...
  for (let i = 0; i < kids.length - 3; i++) {
    if (isHeader(kids[i]) && isHeader(kids[i + 1])) {
      ...
      if (looksLikeList(list1) && looksLikeList(list2)) return { homeList: list1, awayList: list2 };
    }
  }
  return null;
}
...
type LineupRow = {
  ksi_match_id: string; lineup_idx: number; ksi_team_id: string | null;
  ksi_player_id: string | null; player_name: string; shirt_number: number | null;
  squad: "xi" | "bench"; side: "home" | "away"; is_gk: boolean | null; raw: any;
};

function parseLineupList($: cheerio.CheerioAPI, ksiMatchId: string, ksiTeamId: string | null,
  squad: "xi" | "bench", side: "home" | "away", $list: cheerio.Cheerio<any>, startIdx: number): LineupRow[] {
  ...
  for (const el of items) {
    ...
    const spanTexts = $el.find("span").toArray().map((s) => cleanText($(s).text())).filter(Boolean);
    let nameRaw = spanTexts.find((t) => t.includes("(M)")) ??
      spanTexts.sort((a, b) => b.length - a.length)[0] ?? cleanText($el.text());
    nameRaw = stripTrailingMinutes(nameRaw);
    const gk = stripGkMarker(nameRaw);
    const player_name = gk.name || nameRaw || "—";
    rows.push({
      ksi_match_id: ksiMatchId, lineup_idx: startIdx + rows.length,
      ksi_team_id: ksiTeamId, side, squad, ksi_player_id, player_name,
      shirt_number, is_gk: gk.is_gk ? true : null,
      raw: { href: href || null, text: cleanText($el.text()) },
    });
  }
  return rows;
}

function parseLineupsFromReportHtml($: cheerio.CheerioAPI, ksiMatchId: string,
  homeTeamId: string | null, awayTeamId: string | null): LineupRow[] {
  ...
  for (let i = 0; i < target.length; i++) {
    ...
    try {
      ...
      if (dry) {
        ...
        if (lineups.length) {
          const { error: lErr } = await supabase.from("match_lineups")
